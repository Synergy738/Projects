...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        if person in two_genes:
            ...
            geneAmount = 0
        hasTrait = person in have_trait
    
        geneProb = PROBS["gene"][geneAmount]
        traitProb = PROBS["trait"][geneAmount][hasTrait]
        ...
        if (people[person]["mother"] is None) and (people[person]["father"] is None):
            ...
            if geneAmount == 2:
                inheritanceProb = 1 - PROBS["mutation"]
            elif geneAmount == 1:
                inheritanceProb = 0.5
            else:
                inheritanceProb = PROBS["mutation"]

            noInheritance = 1 - inheritanceProb

            if (mother in two_genes) and (father in two_genes):
                probability *= inheritanceProb * inheritanceProb
            elif (mother in two_genes) and (father in one_gene):
                probability *= inheritanceProb * 0.5
            elif (mother in one_gene) and (father in two_genes):
                probability *= 0.5 * inheritanceProb
            else:
                probability *= noInheritance * noInheritance
