...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if self.crossword.overlaps[x, y]:
            ...
            for wordX in self.domains[x]:
                valid = any(wordX[i] == wordY[j] for wordY in self.domains[y])
                if not valid:
                    remove.add(wordX)
            if remove:
    ...
    def ac3(self, arcs=None):
        ...
        vdomains = list(self.domains.values())
        icount = 0
        while icount < len(vdomains):
            x, y = arcs.pop(0)            
            if self.revise(x, y):
                if len(vdomains[icount]) == 0:
                    return False
    ...
    def select_unassigned_variable(self, assignment):
        ...
        variables.sort(key=lambda var: (len(self.domains[var]), -len(self.domains[var])))
        if variables:
            return variables[0]
        else:
            return None
